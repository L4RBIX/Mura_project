...
export function RecordButton({
    ...
    <span className="flex flex-col items-center gap-5">
      ...
      {(label || sublabel) && (
        <span className="flex flex-col items-center gap-1">
          {label && <span className="text-[17px] font-semibold">{label}</span>}
          {sublabel && <span className="text-[14px] text-muted">{sublabel}</span>}
        </span>
      )}
    </span>
  );
  ...
  const sharedClass = cn(
    "group block rounded-[32px] focus-visible:outline-2 focus-visible:outline-offset-8 focus-visible:outline-ink/40",
    className,
  );
